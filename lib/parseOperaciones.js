...
export const SHEET_NAME = "Efecto TC";
...
/**
 * Lee el buffer de un .xlsx y devuelve la hoja "Efecto TC", compartida por
 * parseOperaciones() y parseSensibilidad().
 */
export function readEfectoTCSheet(buffer) {
...
/**
 * A partir de la hoja "Efecto TC", devuelve las operaciones (fecha, usd, pesos)
 * ya deduplicadas y ordenadas. La hoja trae, a la izquierda de la tabla de
 * operaciones (col D-F), celdas de resumen sueltas y, a la derecha (col J en
 * adelante), tablas auxiliares sin relación: se ignoran ambas. Las columnas
 * G/H/I (TC, Efecto, Acumulado) del archivo no se usan — se recalculan
 * siempre en la app para ser robustos a cómo venga la planilla.
 */
export function parseOperaciones(sheet) {
  const rows = [];
